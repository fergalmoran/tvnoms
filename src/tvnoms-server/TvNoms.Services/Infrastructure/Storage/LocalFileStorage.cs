...
public class LocalFileStorage : IFileStorage {
  ...
  public async Task<FileChunkStatus> WriteAsync(string path, Stream chunk, long length, long offset,
  ...
  private string GetFilePath(string path) {
    ...
    var invalidFileNameChars = fileName.Where(c => Path.GetInvalidPathChars().Concat(new[] { '/', '\\' }).Contains(c))
      .ToArray();
    if (invalidFileNameChars.Length > 0)
      throw new ArgumentException($"Invalid characters in file name: {string.Join(", ", invalidFileNameChars)}");

    var directoryNames = Path.GetDirectoryName(path)?.Split(new char[] { '/', '\\' }) ?? Array.Empty<string>();

    foreach (var directoryName in directoryNames) {
      var invalidDirectoryNameChars = directoryName
        .Where(c => Path.GetInvalidPathChars().Concat(new[] { '/', '\\' }).Contains(c)).ToArray();
      if (invalidDirectoryNameChars.Length > 0)
        throw new ArgumentException(
          $"Invalid characters in directory name: {string.Join(", ", invalidDirectoryNameChars)}");
    }
