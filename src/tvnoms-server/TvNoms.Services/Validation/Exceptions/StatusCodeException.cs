...
public class BadRequestException : StatusCodeException {
  ...
  public BadRequestException(IDictionary<string, string[]> errors, string? title = null,
    Exception? innerException = null)
    : base(STATUS_CODE,
      title ??= (errors.Count > 1 ? "One or more validation errors occurred." : errors.First().Value.Humanize()),
      innerException) {
    Errors = errors.AsReadOnly();
  }

  public BadRequestException(string propertyName, string propertyMessage, string? title = null,
    Exception? innerException = null)
...
public class StatusCodeException : InvalidOperationException {
  public StatusCodeException(int statusCode, string? title = null, Exception? innerException = null) : base(
    GetMessage(statusCode), innerException) {
  ...
  protected T? GetDataValue<T>(Exception exception, [CallerMemberName] string propertyName = "") =>
    (T?)exception.Data[propertyName];

  protected void SetDataValue<T>(Exception exception, T value, [CallerMemberName] string propertyName = "") =>
    exception.Data[propertyName] = value;
}
