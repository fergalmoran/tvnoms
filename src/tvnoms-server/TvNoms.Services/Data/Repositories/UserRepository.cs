...
public class UserRepository : AppRepository<User>, IUserRepository {
  ...
  public UserRepository(UserManager<User> userManager, AppDbContext dbContext,
    IOptions<UserSessionOptions> userSessionOptions) : base(dbContext) {
  ...
  public async Task<User> UpdateAsync(User user, DateTimeOffset lastActiveAt,
  ...
  public async Task
    VerifyPhoneNumberTokenAsync(User user, string token, CancellationToken cancellationToken = default) {
    if (user == null) throw new ArgumentNullException(nameof(user));
    if (token == null) throw new ArgumentNullException(nameof(token));
  ...
  public string? GetUserName(ClaimsPrincipal principal) {
    ...
    return principal.FindFirst(_userManager.Options.ClaimsIdentity.UserNameClaimType) is Claim claim
      ? claim.Value
      : null;
  }
  ...
  public Guid? GetUserId(ClaimsPrincipal principal) {
    ...
    return Guid.TryParse(
      principal.FindFirst(_userManager.Options.ClaimsIdentity.UserIdClaimType) is Claim claim ? claim.Value : null,
      out Guid userId)
      ? userId
      : Guid.Empty;
  }
  ...
  public async Task GenerateUserNameAsync(User user, CancellationToken cancellationToken = default) {
    ...
    user.UserName = await Slugifier.GenerateSlugAsync($"{user.FirstName} {user.LastName}".ToLower(),
      userName => _userManager.Users.AnyAsync(_ => _.UserName == userName, cancellationToken));
  }

  public async Task<bool> IsUserNameTakenAsync(User existingUser, string newUserName,
    CancellationToken cancellationToken = default) {
    ...
    var result =
      await _userManager.Users.AnyAsync(_ => _.Id != existingUser.Id && _.NormalizedUserName == normalizedNewUserName,
        cancellationToken);
    return result;
  }

  public async Task<bool> IsEmailTakenAsync(User existingUser, string newEmail,
    CancellationToken cancellationToken = default) {
    if (existingUser == null) throw new ArgumentNullException(nameof(existingUser));
    if (newEmail == null) throw new ArgumentNullException(nameof(newEmail));
    ...
    var result =
      await _userManager.Users.AnyAsync(_ => _.Id != existingUser.Id && _.NormalizedEmail == normalizedNewEmail,
        cancellationToken);
    return result;
  }

  public async Task<bool> IsPhoneNumberTakenAsync(User existingUser, string newPhoneNumber,
    CancellationToken cancellationToken = default) {
    if (existingUser == null) throw new ArgumentNullException(nameof(existingUser));
    if (newPhoneNumber == null) throw new ArgumentNullException(nameof(newPhoneNumber));
    ...
    var result =
      await _userManager.Users.AnyAsync(_ => _.Id != existingUser.Id && _.PhoneNumber == normalizedNewPhoneNumber,
        cancellationToken);
    return result;
  }
  ...
  public async Task AddSessionAsync(User user, UserSessionInfo session, CancellationToken cancellationToken = default) {
    ...
    await _dbContext.AddAsync(new UserSession {
      ...
      AccessTokenHash = TokenHelper.GenerateSHA256Hash(session.AccessToken),
      RefreshTokenHash = TokenHelper.GenerateSHA256Hash(session.RefreshToken),
  ...
  public async Task RemoveSessionAsync(User user, string token, CancellationToken cancellationToken = default) {
    if (!_userSessionOptions.Value.AllowMultipleSessions) {
      await _dbContext.Set<UserSession>().Where(_ => _.UserId == user.Id)
        .ForEachAsync(session => _dbContext.Remove(session), cancellationToken);
    } else {
      var tokenHash = TokenHelper.GenerateSHA256Hash(token);
  ...
  public async Task<User?> GetByAccessTokenAsync(string accessToken, CancellationToken cancellationToken = default) {
    ...
    var accessTokenHash = TokenHelper.GenerateSHA256Hash(accessToken);

    var session = await _dbContext.Set<UserSession>()
      .FirstOrDefaultAsync(_ => _.RefreshTokenHash == accessTokenHash, cancellationToken);
  ...
  public async Task<User?> GetByRefreshTokenAsync(string refreshToken, CancellationToken cancellationToken = default) {
    ...
    var refreshTokenHash = TokenHelper.GenerateSHA256Hash(refreshToken);

    var session = await _dbContext.Set<UserSession>()
      .FirstOrDefaultAsync(_ => _.RefreshTokenHash == refreshTokenHash, cancellationToken);
