...
using TvNoms.Server.Services.Events;
...
namespace TvNoms.Server.Services.Data.Services;
...
public class UserService : IUserService {
  ...
  public UserService(IServiceProvider serviceProvider, IFileStorage fileStorage, IMapper mapper,
    IModelBuilder modelBuilder,
    IMediator mediator, IViewRenderer viewRenderer, IEmailSender emailSender,
    ISmsSender smsSender, IClientContext clientContext, IOptions<FileRuleOptions> fileTypeOptions,
    IMediaRepository mediaRepository, IUserRepository userRepository, IUserSessionFactory userSessionFactory,
  ...
  public async Task SignUpAsync(SignUpForm form) {
    ...
    if (form.UsernameType switch {
      ...
      throw new BadRequestException(nameof(form.Username),
        $"'{form.UsernameType.Humanize(LetterCasing.Sentence)}' is already taken.");
    ...
    await _userRepository.AddToRolesAsync(user,
      (totalUsers == 1) ? new[] { Role.Admin, Role.Member } : new[] { Role.Member }, cancellationToken);
  ...
  public async Task<UserWithSessionModel> SignInAsync(SignInForm form) {
    ...
    if (user == null)
      throw new BadRequestException(nameof(form.Username),
        $"'{form.UsernameType.Humanize(LetterCasing.Sentence)}' does not exist.");

    if (!await _userRepository.CheckPasswordAsync(user, form.Password, cancellationToken))
      throw new BadRequestException(nameof(form.Password),
        $"'{nameof(form.Password).Humanize(LetterCasing.Sentence)}' is not correct.");
  ...
  public async Task<UserWithSessionModel> SignInWithAsync(SignUpWithForm form) {
    ...
    await _userRepository.AddLoginAsync(user,
      new UserLoginInfo(form.ProviderName, form.ProviderKey, form.ProviderDisplayName), cancellationToken);
  ...
  public async Task SignOutAsync(SignOutForm form) {
    ...
    if (user == null)
      throw new BadRequestException(nameof(form.RefreshToken),
        $"'{nameof(form.RefreshToken).Humanize(LetterCasing.Sentence)}' is not valid.");
  ...
  public async Task SendUsernameTokenAsync(SendUsernameTokenForm form) {
    ...
    if (form.UsernameType == ContactType.Email) {
      ...
      var message = new EmailMessage() {
        ...
        Body = await _viewRenderer.RenderAsync("/Email/VerifyUsername",
          (user, new VerifyUsernameForm { Username = form.Username, Code = code }), cancellationToken)
      };
      ...
      var message = await _viewRenderer.RenderAsync("/Text/VerifyUsername",
        (user, new VerifyUsernameForm { Username = form.Username, Code = code }), cancellationToken);
      await _smsSender.SendAsync(form.Username, message, cancellationToken);
    } else {
      throw new InvalidOperationException(
        $"The value '{form.UsernameType}' of enum '{nameof(ContactType)}' is not supported.");
    }
  }
  ...
  public async Task VerifyUsernameAsync(VerifyUsernameForm form) {
    ...
    if (user == null)
      throw new BadRequestException(nameof(form.Username),
        $"'{form.UsernameType.Humanize(LetterCasing.LowerCase)}' does not exist.");
    ...
    try {
      ...
      throw new BadRequestException(nameof(form.Code),
        $"'{nameof(form.Code).Humanize(LetterCasing.Sentence)}' is not valid.", innerException: ex);
    }
  }
  ...
  public async Task SendPasswordResetTokenAsync(SendPasswordResetTokenForm form) {
    ...
    if (form.UsernameType == ContactType.Email) {
      ...
      var message = new EmailMessage() {
        ...
        Body = await _viewRenderer.RenderAsync("/Email/ResetPassword",
          (user, new ResetPasswordForm { Username = form.Username, Code = code }), cancellationToken)
      };

      await _emailSender.SendAsync(message, cancellationToken);
    } else if (form.UsernameType == ContactType.PhoneNumber) {
      var code = await _userRepository.GeneratePasswordResetTokenAsync(user, cancellationToken);
      var message = await _viewRenderer.RenderAsync("/Text/ResetPassword",
        (user, new ResetPasswordForm { Username = form.Username, Code = code }), cancellationToken);
      await _smsSender.SendAsync(form.Username, message, cancellationToken);
    } else {
      throw new InvalidOperationException(
  ...
  public async Task ChangePasswordAsync(ChangePasswordForm form) {
    ...
    var currentUser = _clientContext.UserId != null
      ? await _userRepository.GetByIdAsync(_clientContext.UserId.Value, cancellationToken)
      : null;
  ...
  public async Task<UserPageModel> GetUsersAsync(UserCriteria criteria, long offset, int limit) {
    ...
    var page = (await _userRepository.GetManyAsync(offset, limit, predicate: predicate,
  ...
  public async Task EditCurrentUserAsync(EditUserForm form) {
    ...
    if (await _userRepository.IsUserNameTakenAsync(currentUser, form.UserName, cancellationToken))
      throw new BadRequestException(nameof(form.UserName),
        $"'{(nameof(form.UserName).ToLower()).Humanize(LetterCasing.Sentence)}' is already taken.");

    if (string.IsNullOrWhiteSpace(form.Email) && currentUser.EmailRequired)
      throw new BadRequestException(nameof(form.Email),
        $"'{(nameof(form.Email).ToLower()).Humanize(LetterCasing.Sentence)}' must not be empty.");

    if (string.IsNullOrWhiteSpace(form.PhoneNumber) && currentUser.PhoneNumberRequired)
      throw new BadRequestException(nameof(form.PhoneNumber),
        $"'{(nameof(form.PhoneNumber).ToLower()).Humanize(LetterCasing.Sentence)}' must not be empty.");
  ...
  public async Task<MediaModel> GetCurrentUserAvatarAsync() {
    ...
    var avatar = currentUser.AvatarId != null
      ? await _mediaRepository.GetByIdAsync(currentUser.AvatarId.Value, cancellationToken)
      : null;
